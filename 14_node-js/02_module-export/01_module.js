/*
    모듈이란?
    자바 스크립트는 웹페이지의 보조적인 기능을 수행하기 위해 만들어진 한정적인 언어였다.
    따라서 다른 언어에 비해 부족한 부분이 있는 것이 사실이며, 그 대표적인 것이 모듈 개념이다.
    브라우저 상에서 동작하는 js는 script 태그를 이용하여 여러 개의 script 파일을 하나로 합쳐 사용한다.
    이 때 js 파일들은 동일한 유효범위(scrope)를 가지게 되어 전역변수 중복과 같은 문제가 발생할 수 있었다.

    Node.js를 이용해 js를 client에 국한된 것이 아닌 server side에서도 사용하고자 할 때,
    ...
    commonJS에 포함되지 않아 트랜스 파일링이 필요하다.)

    javascript의 모듈 시스템은 대표적으로 ESM, CommonJS라는 두가지 기술이 존재한다. 
...
console.log(`너비 2 높이 3인 직사각형의 둘레 : ${ractangle.perimeter(2,3)}`);